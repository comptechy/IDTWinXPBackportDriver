@echo off
rem Build dstest.exe - user-mode x64 console app for Windows XP x64.
rem
...
cd /d "%~dp0"
if exist dstest.exe del dstest.exe
if exist dstest.obj del dstest.obj
...
if errorlevel 1 goto :fail
if not exist dstest.exe goto :fail

echo.
echo ==== BUILD OK ====
dir dstest.exe | findstr /i dstest
endlocal
exit /b 0
